#!/usr/bin/env bash
# Codex 的 Goal 模式 runner：循环跑到额度耗尽，耗尽后退避等窗口重置再续。
# 前提：已 gh auth login；本脚本在仓库克隆目录的上一级或仓库内运行。
# 注意：下面的启动 flag 以 Codex CLI 当前版本 --help 为准，可能需微调。
set -uo pipefail
cd "$(dirname "$0")/.."

PROMPT="读取 AGENTS.md 和 COORDINATION.md。执行一次 Goal LOOP 迭代：
...
while true; do
  ...
  if codex exec --full-auto "$PROMPT"; then
    ...
    echo "[$(date)] Codex 调用失败（多半是额度耗尽），退避 ${backoff}s 后重试"
    sleep "$backoff"
