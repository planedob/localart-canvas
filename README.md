...
M0、M1 与 M2 第一阶段 Electron 桌面壳已完成。M2 第二阶段已实现 Ollama/OpenAI-compatible 统一聊天接口、Primary/Backup fallback、侧栏配置、JSON/ZIP/PNG 导出、历史版本恢复和基础快捷键，真实云端点仍待人工密钥验收。真实 Flux.2 klein 本地闭环已在 Apple M4 16GB 的浏览器和打包 Electron 应用中验证。代码钩子和 API 调研见 [`docs/notes.md`](docs/notes.md)，进度和手测记录见 [`PROGRESS.md`](PROGRESS.md)。
