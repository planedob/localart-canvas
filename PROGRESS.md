...
- 聊天接口固定使用非流式响应（`stream: false`）。用于“生成 ComfyUI 提示词”这种短输出可接受；普通长聊天会等待整段返回。流式显示留作后续。
- 多模态请求依赖用户选择的模型能力。若 Backup/Custom 选择纯文本模型但请求携带画布截图，ProviderError 会以安全错误返回给 UI；人工验收需确认错误是可读提示，而不是原始 provider 报文。
- Primary 的 401/403、模型不存在、请求格式错误等配置类问题不会切到 Backup。这是有意暴露配置错误，避免静默掩盖坏 key 或产生意外云端调用。
