...
M0、M1、M2 主要代码工作已经完成。现在最适合交 Claude 做阶段验收；剩余项目主要是人工环境验证、真实云端 key 验收、签名公证和发布策略，不是继续闷头写代码能完全解决的部分。

当前建议验收范围：M2 代码层 + 自动化证据 + 人工验收清单。
...
- 分支：`main`
- handoff 基准提交：`b974793 docs: note model routing caveats`
...
- GitHub CI：`28080224740`，通过
- Desktop package：`28080224762`，通过
  - macOS package：通过
  - Windows package：通过
  - Linux package：通过
...
- M1 本地核心闭环：100%
- M2 代码与自动化：约 90–95%
- M2 人工验收：约 60–70%，因为云 key、Windows/Linux GUI 真机、签名公证需要 Eric 或有凭据/设备的人操作
- 距离“可交 Claude 验收”：已达到
- 距离“可公开发布给普通用户”：约 85–90%，主要差签名、公证、真实多平台 GUI 和 release 流程

如果只做 Claude 阶段验收，预计还需要 0.5–1 天人工操作：录屏、真实云 key 验收、Windows/Linux GUI 抽测。  
...
git status --short
git log --oneline -5
gh run view 28080224740 --repo planedob/localart-canvas --json status,conclusion,url
gh run view 28080224762 --repo planedob/localart-canvas --json status,conclusion,url
```
...
- handoff 基准提交：`b974793`
- CI：`28080224740`，通过
- Desktop package：`28080224762`，三平台通过
- `PROGRESS.md` 摘要
- `docs/M2-剩余人工验收清单.md`
- M1/M2 录屏或截图
...
- M1 闭环是否仍能走通。
- Electron 壳是否满足 M2S1。
- 模型 Provider 设计是否符合“OpenAI-compatible only + AIBuff 默认 + 本地 Ollama 同抽象”。
- fallback、密钥、本地配置、错误提示是否符合红线。
- 导出、历史、快捷键、右键菜单是否具备可验收路径。
...
```text
你接手 LocalArt Canvas。先读 README.md、PROGRESS.md、docs/M2-剩余人工验收清单.md、docs/superpowers/specs/2026-06-23-model-provider-design.md、docs/M2S1-验收交接.md。当前 main handoff 基准提交是 b974793，handoff 文档提交见 git log 最新一条，CI 28080224740 通过，Desktop package 28080224762 三平台通过。不要先改代码，先核验状态和人工阻塞项。红线：不碰 secrets/token/CI 凭据，不 force-push，不删历史/分支/仓库，不改仓库设置，不写非 GitHub 远端。下一步优先完成 Claude 验收材料、真实云模型 key 人工验收、Windows/Linux GUI 真机验收；签名公证和发布需要 Eric 提供凭据和决策。
```

## 11. 当前交付建议

现在可以让 Claude 验收。验收时把人工阻塞项明确标成“待人工验证”，不要把签名、公证、真实 key、Windows/Linux 真机当作代码未完成项。
